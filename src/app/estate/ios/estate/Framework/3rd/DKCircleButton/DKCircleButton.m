...
#define DKCircleButtonBorderWidth 6.0f
...
- (void)setupUI {
    ...
    CGRect frame = self.frame;
        ...
        _highLightView.backgroundColor = [UIColor colorWithRed:0x33/255.f green:0xce/255.f blue:0x88/255.f alpha:0.5];//[UIColor colorWithWhite:1 alpha:0.5];
        
        _borderColor = [UIColor colorWithRed:0x33/255.f green:0xce/255.f blue:0x88/255.f alpha:1];//[UIColor whiteColor];
        _animateTap = YES;
...
- (void)updateMaskToBounds:(CGRect)maskBounds {
    ...
    self.layer.shadowRadius = 4;//设置阴影的圆角
    if (!self.selected) {
    ...
    maskLayer.cornerRadius = CGRectGetHeight(maskBounds) / 2.0;
    self.layer.borderColor = [self.borderColor colorWithAlphaComponent:0.7].CGColor;
//    self.layer.borderWidth = 0;//self.borderSize;
...
- (void)triggerAnimateTap {
    ...
    circleShape.fillColor = [UIColor clearColor].CGColor;
    circleShape.opacity = 0;
    circleShape.strokeColor = _highLightView.backgroundColor.CGColor;//self.borderColor.CGColor;
    ...
    CABasicAnimation *scaleAnimation = [CABasicAnimation animationWithKeyPath:@"transform.scale"];
    scaleAnimation.fromValue = [NSValue valueWithCATransform3D:CATransform3DIdentity];
    scaleAnimation.toValue = [NSValue valueWithCATransform3D:CATransform3DMakeScale(1.25, 1.25, 1)];
