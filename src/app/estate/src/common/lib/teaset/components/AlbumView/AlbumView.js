...
export default class AlbumView extends Component {
  ...
  changeIndex(newIndex) {
    ...
    const sheet = this.refs[`sheet${index}`]
    const nextSheet = this.refs[`sheet${newIndex}`]
    const toPosition = newIndex > index ? 'left' : 'right'
  ...
  onTransforming(translateX, translateY, scale) {
    ...
    const { index } = this.state
    const { x, y, width, height } = this.refs[`sheet${index}`].contentLayout
    let ltx = translateX,
      rtx = translateX
  ...
  onWillMagnetic(translateX, translateY, scale, newX, newY, newScale) {
    ...
    const triggerWidth = this.layout.width / 3
