...
export default class Carousel extends Component {
  ...
  // 初始化轮播参数
  initByProps(props) {
    const { children, carousel, direction, startIndex, cycle } = props
  ...
  render() {
    ...
    if (width > 0 && height > 0) {
      let fixStyle
      if (horizontal) fixStyle = { width: width * this.cardCount, height }
      else fixStyle = { width, height: height * this.cardCount }
