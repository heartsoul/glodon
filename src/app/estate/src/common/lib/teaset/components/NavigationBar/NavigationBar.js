...
export default class NavigationBar extends Component {
  ...
  buildProps() {
    ...
    const { left: leftInset, right: rightInset } = Theme.screenInset
    style = [
      {
    ...
    const { leftViewWidth, rightViewWidth } = this.state
    const barPaddingLeft = fs.paddingLeft
      ? fs.paddingLeft
      : fs.padding ? fs.padding : 0
    const barPaddingRight = fs.paddingRight
      ? fs.paddingRight
      : fs.padding ? fs.padding : 0
    ...
    const leftRightViewStyle = { opacity: this.state.barOpacity }
    ...
    if (typeof title === 'string') {
      title = (
        <this.constructor.Title
          style={[
            { textAlign: titleTextAlign, color: Theme.navTitleColor },
          ].concat(titleStyle)}
          text={title}
        />
      )
  ...
  checkBarHidden(hidden, animated) {
    const { barTop, barOpacity } = this.state
    const barTopValue = hidden ? -this.barHeight : 0
    const barOpacityValue = hidden ? 0 : 1
