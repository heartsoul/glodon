...
export default class AlbumView extends Component {
  ...
  getImageSizeInfo(index) {
    ...
    const imageInfo = this.state.imageInfos[index]
  ...
  scrollToImage(newIndex) {
    ...
    const directionFactor = newIndex < index ? -1 : 1
    const lrAnimated =
      newIndex < index ? this.state.leftTranslateX : this.state.rightTranslateX
    const lrValue = -(width + space) * directionFactor
    const distance = lrValue - lrAnimated._value
    const tvValue = this.refs.transformView.state.translateX._value + distance
  ...
  checkLeftRight() {
    ...
    if (leftIndex != index - 1 || rightIndex != index + 1) {
      const { width } = this.state.layout
      const { scaleWidth } = this.getImageSizeInfo(index)
      const imageInfo = imageInfos[index]
      const tx =
  ...
  onTransforming(translateX, translateY, scale) {
    const saveScale = this.saveScale
    this.saveScale = scale
    if (scale < 1 || (saveScale && scale < saveScale)) {
      return
    }

    const { x, y, width, height } = this.refs.transformView.contentLayout
    let ltx = translateX,
      rtx = translateX
  ...
  onDidTransform(translateX, translateY, scale) {
    this.saveScale = scale
    const { index, imageInfos } = this.state
  ...
  onWillMagnetic(translateX, translateY, scale, newX, newY, newScale) {
    ...
    const triggerWidth = this.state.layout.width / 3
    ...
    if (scale < 1) {
      ...
      const distance = newX - translateX
      const newltx = ltx + distance
      const newrtx = rtx + distance
      Animated.parallel([
  ...
  renderLeftImage() {
    const { images, thumbs, space } = this.props
    const { leftIndex, imageInfos, leftTranslateX } = this.state
    if (leftIndex < 0 || leftIndex >= images.length) return null

    const { loadStatus, translateY, scale } = imageInfos[leftIndex]
    const { width, height } = this.state.layout
    const { scaleWidth } = this.getImageSizeInfo(leftIndex)
    const cy = height / 2
    const top = -cy * scale + translateY + cy
    const viewStyle = {
      position: 'absolute',
      left: -(width + space),
      top: 0,
  ...
  renderRightImage() {
    const { images, thumbs, space } = this.props
    const { rightIndex, imageInfos, rightTranslateX } = this.state
    if (rightIndex < 0 || rightIndex >= images.length) return null

    const { loadStatus, translateY, scale } = imageInfos[rightIndex]
    const { width, height } = this.state.layout
    const { scaleWidth } = this.getImageSizeInfo(rightIndex)
    const cy = height / 2
    const top = -cy * scale + translateY + cy
    const viewStyle = {
      position: 'absolute',
  ...
  renderImage() {
    const { images, thumbs, space, maxScale, onPress, onLongPress } = this.props
    const { index, imageInfos } = this.state
    const { loadStatus, width, height } = imageInfos[index]

    const { initWidth, initHeight } = this.getImageSizeInfo(index)
    let imageSource
    switch (loadStatus) {
      case 1:
