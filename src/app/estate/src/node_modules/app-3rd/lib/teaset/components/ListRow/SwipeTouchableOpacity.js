...
export default class SwipeTouchableOpacity extends TouchableOpacity {
  ...
  replaceSuperFunction() {
    ...
    const touchableHandleActivePressOut = this.touchableHandleActivePressOut
    this.touchableHandleActivePressOut = e => {
      this.swipeOver()
      touchableHandleActivePressOut(e)
    }

    const touchableHandlePress = this.touchableHandlePress
    this.touchableHandlePress = e => {
      if (!this.checkPress()) touchableHandlePress(e)
    }

    const touchableHandleLongPress = this.touchableHandleLongPress
  ...
  swiping(e) {
    ...
    const dx = touches[0].pageX - prevTouches[0].pageX
  ...
  buildProps() {
    ...
    style = []
      .concat(style)
      .concat({ transform: [{ translateX: this.state.translateX }] })
