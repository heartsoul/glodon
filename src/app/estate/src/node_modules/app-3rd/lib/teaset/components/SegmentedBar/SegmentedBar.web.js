...
export default class SegmentedBar extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const nextItemsLayout = this.makeArray(
      this._itemsLayout,
      nextProps.children
    )
  ...
  makeArray(olders, items, empty = { x: 0, y: 0, width: 0, height: 0 }) {
    if (items instanceof Array)
      return items.map(
        (item, index) => (index < olders.length ? olders[index] : empty)
      )
  ...
  updateIndicator() {
    ...
    const indicatorXValue = this.indicatorXValue
    const indicatorWidthValue = this.indicatorWidthValue
    if (
  ...
  renderItem(item, index) {
    const saveOnLayout = item.props.onLayout
    const newItem = React.cloneElement(item, {
      active: index === this.activeIndex ? 'true' : 'false',
      onLayout: e => {
  ...
  renderIndicator() {
    ...
    const style = {
      backgroundColor: indicatorLineColor || Theme.sbIndicatorLineColor,
      position: 'absolute',
