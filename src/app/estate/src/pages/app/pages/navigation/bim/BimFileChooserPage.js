...
export default class BimFileChooser extends Component {
    ...
    _itemClick = (item,index) => {
        ...
        if(item.value.folder === true) {
            storage.pushNext(navigator,"BimFileChooserPage",{fileId:item.value.fileId,dataType:this.state.dataType, pageType:this.state.pageType});
        } else {
            MODELAPI.getModelBimFileToken(this.state.projectId,this.state.latestVersion,item.value.fileId).then((responseData)=>{
                ...
                if(this.state.dataType === '图纸文件'){
                    storage.pushNext(navigator,"RelevantBlueprintPage",{title:item.value.name, fileId:item.value.fileId,pageType:this.state.pageType});
                }else{
